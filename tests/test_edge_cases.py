...
def test_build_nested_dict_edge_cases():
    ...
    schema_shared = Schema(
        version="1.0",
        values=[
            SchemaValue(key="db-host", path="db.host", description="", type="string", required=True),
            SchemaValue(key="db-port", path="db.port", description="", type="number", required=True),
            SchemaValue(key="app-name", path="app.name", description="", type="string", required=True),
        ]
    )
    
    result = build_nested_dict(
        {"db-host": "localhost", "db-port": 5432, "app-name": "test"},
        schema_shared
    )
    assert result == {
...
def test_schema_validation_edge_cases(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(app, ["validate", "--env", "test", "--schema", "optional-schema.json"])
        assert result.exit_code == 0
        
        # Generate with optional values should include defaults
        result = runner.invoke(app, ["generate", "--env", "test", "--schema", "optional-schema.json"])
        assert result.exit_code == 0
...
def test_type_validation_edge_cases(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        schema_data = {
            "version": "1.0",
            "values": [
                {"key": "str", "path": "str", "description": "", "type": "string", "required": False},
                {"key": "num", "path": "num", "description": "", "type": "number", "required": False},
                {"key": "bool", "path": "bool", "description": "", "type": "boolean", "required": False},
                {"key": "arr", "path": "arr", "description": "", "type": "array", "required": False},
                {"key": "obj", "path": "obj", "description": "", "type": "object", "required": False},
            ]
        }
        with open("schema.json", "w") as f:
            json.dump(schema_data, f)
...
def test_file_permission_edge_cases(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(app, ["values", "set", "key", "value", "--env", "test", 
                                   "--values", str(readonly_dir / "values.json")])
...
def test_unicode_and_special_characters(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(app, ["values", "set", "unicode-test", "Héllo Wörld", "--env", "test"])
        assert result.exit_code == 0
...
def test_large_data_structures(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(app, ["values", "set", "large-array", json.dumps(large_array), "--env", "test"])
        assert result.exit_code == 0
        
        # Create large object (many keys)
        large_object = {f"key-{i}": f"value-{i}" for i in range(50)}
        result = runner.invoke(app, ["values", "set", "large-object", json.dumps(large_object), "--env", "test"])
        assert result.exit_code == 0
