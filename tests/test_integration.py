...
def test_complete_vendor_workflow(tmp_path, monkeypatch):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(app, schema_commands[0], 
                             input="app-name\napp.name\nApplication name\nstring\ny\ny\nmyapp\nn\n")
        assert result.exit_code == 0
        
        # Add port (required, no default)
        result = runner.invoke(app, schema_commands[1],
                             input="port\napp.port\nApplication port\nnumber\ny\nn\nn\n")
        assert result.exit_code == 0
        
        # Add debug (optional with default)
        result = runner.invoke(app, schema_commands[2],
                             input="debug\napp.debug\nDebug mode\nboolean\nn\ny\nfalse\nn\n")
        assert result.exit_code == 0
        
        # Add api-key (required, sensitive)
        result = runner.invoke(app, schema_commands[3],
                             input="api-key\napp.apiKey\nAPI key\nstring\ny\nn\ny\n")
        assert result.exit_code == 0
        
        # Add features (optional array)
        result = runner.invoke(app, schema_commands[4],
                             input="features\napp.features\nFeature flags\narray\nn\nn\nn\n")
        assert result.exit_code == 0
...
def test_complete_customer_workflow(tmp_path, monkeypatch):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        # Set optional array
        result = runner.invoke(app, ["values", "set", "features", '["auth", "logging"]', "--env", "prod"])
        assert result.exit_code == 0
...
def test_multi_environment_workflow(tmp_path, monkeypatch):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(app, ["values", "set", "database-host", "dev-db.local", "--env", "dev"])
        assert result.exit_code == 0
        ...
        result = runner.invoke(app, ["values", "set", "database-host", "prod-db.example.com", "--env", "prod"])
        assert result.exit_code == 0
...
def test_complex_data_types_workflow(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        # Set array value
        result = runner.invoke(app, ["values", "set", "hosts", 
                                   '["api.example.com", "www.example.com"]', "--env", "prod"])
        assert result.exit_code == 0
        
        # Set object value (override default)
        result = runner.invoke(app, ["values", "set", "resources", 
                                   '{"requests": {"cpu": "200m", "memory": "256Mi"}}', "--env", "prod"])
        assert result.exit_code == 0
        
        # Set nested object
        result = runner.invoke(app, ["values", "set", "annotations",
                                   '{"nginx.ingress.kubernetes.io/rewrite-target": "/"}', "--env", "prod"])
        assert result.exit_code == 0
        ...
        assert generated["ingress"]["annotations"]["nginx.ingress.kubernetes.io/rewrite-target"] == "/"