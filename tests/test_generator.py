...
def test_build_nested_dict_simple():
    ...
    schema = Schema(
        version="1.0",
        values=[
            SchemaValue(key="host", path="database.host", description="", type="string", required=True),
            SchemaValue(key="port", path="database.port", description="", type="number", required=True),
...
def test_build_nested_dict_deep_paths():
    ...
    schema = Schema(
        version="1.0",
        values=[
            SchemaValue(key="cpu", path="resources.requests.cpu", description="", type="string", required=True),
            SchemaValue(key="memory", path="resources.requests.memory", description="", type="string", required=True),
        ]
    )
...
def test_generate_values_with_defaults():
    ...
    schema = Schema(
        version="1.0",
        values=[
            SchemaValue(key="replicas", path="deployment.replicas", description="", type="number", required=False, default=3),
            SchemaValue(key="port", path="service.port", description="", type="number", required=True),
        ]
    )
...
def test_generate_values_with_secrets(monkeypatch):
    ...
    schema = Schema(
        version="1.0",
        values=[
            SchemaValue(key="password", path="database.password", description="", type="string", required=True, sensitive=True),
        ]
    )
    
    values = {
        "password": {"type": "env", "name": "DB_PASSWORD"}
    }
...
def test_generate_values_arrays_and_objects():
    ...
    schema = Schema(
        version="1.0",
        values=[
            SchemaValue(key="hosts", path="ingress.hosts", description="", type="array", required=True),
            SchemaValue(key="annotations", path="ingress.annotations", description="", type="object", required=True),
        ]
    )
...
def test_generate_command_missing_env_var(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        values_data = {
            "secret": {"type": "env", "name": "MISSING_VAR"}
        }
